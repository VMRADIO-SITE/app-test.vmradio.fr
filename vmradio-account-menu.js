(() => {
  ...
  function saveUser(next){if(!next)return;user={...next,display_name:displayName(next)};try{localStorage.setItem(USER_KEY,JSON.stringify(user));localStorage.setItem(TOP_NAME_KEY,displayName(user))}catch{}}
  function clearLocalAccount(){try{localStorage.removeItem(USER_KEY);localStorage.removeItem(TOP_NAME_KEY)}catch{}}
  ...
  function injectStyles(){
    ...
    const s=document.createElement('style');s.id='vm-account-menu-style';s.textContent=`
...
#vm-account-home.open #vm-account-home-panel{max-height:620px;opacity:1;transform:translateY(0);pointer-events:auto;margin-top:8px}
...
.vm-account-password-title{margin:12px 0 3px;padding-top:10px;border-top:1px solid rgba(255,255,255,.07);font:900 10px Arial,sans-serif;color:#eadfff}
.vm-account-password-note{margin:0 0 5px;color:#83798b;font:700 8px/1.4 Arial,sans-serif}
...
`;document.head.appendChild(s)}
  ...
  function homeLogo(){const candidates=[document.querySelector('.top img'),document.querySelector('header img'),document.querySelector('.brand img'),document.querySelector('.logo img'),[...document.querySelectorAll('img')].find(img=>/vm\s*radio/i.test(img.alt||'')&&!img.closest('#vmWelcomeSplash'))].filter(Boolean);return candidates[0]||null}

  function ensureMenu(){
    injectStyles();if(!user)user=readUser();if(!user)return null;
    let root=document.getElementById('vm-account-home');
    if(!root){
      root=document.createElement('div');root.id='vm-account-home';root.innerHTML=`
        <button id="vm-account-home-trigger" type="button" aria-expanded="false"><span class="vm-dot"></span><span id="vm-account-home-name"></span><span class="vm-chevron">⌄</span></button>
        <div id="vm-account-home-panel"><div class="vm-account-panel-card">
          ...
          <div id="vm-account-edit">
            ...
            <div class="vm-account-password-title">Modifier mon mot de passe</div>
            <p class="vm-account-password-note">Laisse ces champs vides si tu ne souhaites pas changer ton mot de passe.</p>
            <label>Mot de passe actuel<input id="vm-account-current-password" type="password" autocomplete="current-password"></label>
            <label>Nouveau mot de passe<input id="vm-account-new-password" type="password" minlength="8" autocomplete="new-password"></label>
      ...
      const logo=homeLogo(),host=logo?.parentElement;if(host)host.insertAdjacentElement('afterend',root);else document.querySelector('.app')?.prepend(root);
      const trigger=root.querySelector('#vm-account-home-trigger'),edit=root.querySelector('#vm-account-edit'),status=root.querySelector('#vm-account-menu-status');
      trigger.onclick=()=>{const open=!root.classList.contains('open');root.classList.toggle('open',open);trigger.setAttribute('aria-expanded',open?'true':'false');if(!open){edit.classList.remove('open');status.textContent=''}};
      root.querySelector('#vm-account-edit-open').onclick=()=>{edit.classList.add('open');root.querySelector('#vm-account-edit-name').value=displayName(user);root.querySelector('#vm-account-edit-email').value=clean(user?.email);root.querySelector('#vm-account-current-password').value='';root.querySelector('#vm-account-new-password').value='';status.textContent=''};
      ...
      root.querySelector('.vm-account-save').onclick=async()=>{
        const name=clean(root.querySelector('#vm-account-edit-name').value),email=clean(root.querySelector('#vm-account-edit-email').value).toLowerCase(),currentPassword=root.querySelector('#vm-account-current-password').value,newPassword=root.querySelector('#vm-account-new-password').value,btn=root.querySelector('.vm-account-save');
        ...
        if((currentPassword||newPassword)&&!currentPassword){status.textContent='Entre ton mot de passe actuel.';return}
        if(newPassword&&newPassword.length<8){status.textContent='Le nouveau mot de passe doit contenir au moins 8 caractères.';return}
        btn.disabled=true;status.textContent='Enregistrement…';
        try{
          const {r,d}=await api('/profile',{method:'POST',body:JSON.stringify({name,email,currentPassword,newPassword})});
          if(!r.ok||!d.ok)throw Error(d.error||'Impossible de modifier le compte.');
          saveUser(d.user);render();edit.classList.remove('open');status.textContent=d.passwordChanged?'✓ Informations et mot de passe mis à jour':'✓ Informations mises à jour';
        }catch(e){status.textContent=String(e?.message||e)}finally{btn.disabled=false}
      };
      root.querySelector('#vm-account-logout').onclick=async()=>{const btn=root.querySelector('#vm-account-logout');btn.disabled=true;status.textContent='Déconnexion…';try{await api('/logout',{method:'POST'})}catch{}clearLocalAccount();user=null;root.remove();location.reload()};
      root.querySelector('#vm-account-delete').onclick=async()=>{
        const btn=root.querySelector('#vm-account-delete');
        if(user?.managerAccess){status.textContent='Ce compte Manager doit être supprimé depuis l’administration.';return}
        if(!confirm('Supprimer définitivement ton compte VM RADIO ? Cette action est irréversible.'))return;
        const confirmation=prompt('Pour confirmer, écris SUPPRIMER');if(confirmation!=='SUPPRIMER'){status.textContent='Suppression annulée.';return}
        btn.disabled=true;status.textContent='Suppression du compte…';
        try{const {r,d}=await api('/delete-account',{method:'POST',body:JSON.stringify({confirm:'SUPPRIMER'})});if(!r.ok||!d.ok)throw Error(d.error||'Impossible de supprimer le compte.');clearLocalAccount();user=null;root.remove();location.reload()}catch(e){status.textContent=String(e?.message||e);btn.disabled=false}
      };
    }
    render();return root;
  }
  ...
  function render(){
    const root=document.getElementById('vm-account-home');if(!root||!user)return;const name=displayName(user);
    root.querySelector('#vm-account-home-name').textContent=name;root.querySelector('#vm-account-panel-name').textContent=name;root.querySelector('#vm-account-panel-email').textContent=clean(user?.email);
    const deleteButton=root.querySelector('#vm-account-delete');if(deleteButton)deleteButton.style.display=user?.managerAccess?'none':'';
